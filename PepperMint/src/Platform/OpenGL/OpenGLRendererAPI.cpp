...
void OpenGLMessageCallback(unsigned, unsigned, unsigned, unsigned int iSeverity, int, const char* iMessage, const void*) {
	switch (iSeverity) {
		case GL_DEBUG_SEVERITY_HIGH:         PM_CORE_FATAL(iMessage); return;
		case GL_DEBUG_SEVERITY_MEDIUM:       PM_CORE_ERROR(iMessage); return;
		case GL_DEBUG_SEVERITY_LOW:          PM_CORE_WARN(iMessage); return;
		case GL_DEBUG_SEVERITY_NOTIFICATION: PM_CORE_TRACE(iMessage); return;
		default:
